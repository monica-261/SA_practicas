...
from .serializers import solicitarPedidoSerializer
...
def solicitarPedido(request):
    ...
    # Se válida si la petición es un post
        ...
        serializer = solicitarPedidoSerializer(data=request.data)
        # Se válida si la petición es correcta
        if serializer.is_valid():
            with open('logs.txt', 'w') as f:
                f.write(str(today) + ' El pedido se ha realizado\n')
            ...
            return Response('El pedido se ha realizado con éxito', status=status.HTTP_201_CREATED)
